...
| `data/submissions.json` | The public **index**: one row per machine, totals only, no daily rows and (since M14.1) no identity block. A returning submitter's row is updated in place, never duplicated. Measured: **544 bytes per row**, whether or not that machine has a fingerprint — it was 1,857 bytes for a fingerprinted row while the 16 anchor digests were still in it |
...
data/submissions.json   {"schema_version": 2, "submissions": [
                          {id, submitted_at, updated_at?, nickname, plan, client,
                           ...
                           daily_days, detail, script_version} ]}
...
An index row with **no** entry in `data/identity.json` is a normal state, not a broken one: the row committed before M13 has no fingerprint to carry. The reverse — an entry keyed by an id the index does not list — is an error, and `tests/dataset_validate.py` rejects it.
...
Why the split, measured on the dataset as it stood: one daily row is 99 bytes and one machine-year is ~35 KB, so the old single file would have been ~3.4 MB at 100 submitters and ~34 MB at 1000 — and long before that the *write* path would have stopped, because the GitHub Contents API does not return inline content for a file over 1 MB and the read would have parsed empty. (That cap is GitHub's documented behaviour and the failure follows from the code path that read through it; it was not reproduced against the live API, because the fix was to leave that endpoint rather than to walk up to it.) Storage therefore also moved to the Git Data API (blob → tree → commit → ref), which has no such cap and which can put all four files in one commit. A moved branch (HTTP 422 on the ref update) earns one re-read, and the match, the merge, the identity map and the fleet delta all re-run against the fresh content.
...
제출 1건은 여전히 커밋 1건이고, 네 파일이 GitHub Git Data API로 한 트리·한 커밋에 함께 들어간다. Contents API를 떠난 이유는 1MB 인라인 상한이다. 그 상한에 걸리면 읽기가 빈 값으로 파싱되면서 제출이 통째로 실패하고 화면에는 이유가 뜨지 않는다. 색인 행의 합계는 자기 상세 파일의 합과 정확히 같고 `data/daily.json`은 모든 상세 파일의 합과 정확히 같다. 색인 행에 identity 항목이 없는 건 정상이다. M13 이전에 들어온 행은 지문 자체가 없다. 반대로 색인에 없는 id의 identity 항목은 오류다. `tests/dataset_validate.py`가 그 규칙 자체이고 커밋된 데이터와 계약 테스트 양쪽에서 돌린다.
