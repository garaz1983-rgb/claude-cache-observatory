...
- **The dataset is three files and they must agree** (M14). `data/submissions.json` is the index (totals, no daily rows), `data/daily.json` is the fleet-wide series summed per calendar date, and `data/subs/<id>.json` is one submission's daily detail. The rule that replaced "add up the one file by hand" is: **an index row's totals equal the sum of its own detail file, and `data/daily.json` equals the sum across all detail files.** `tests/dataset_validate.py` is that rule; run it after touching anything under `data/` and after any change to the write path. Do not add a fourth data file, a derived field the page recomputes differently, or a renderer that patches a disagreement — a number the reader cannot re-derive from a file they can open is the thing this site exists not to be.
- **A submission is one commit, containing all three files.** `functions/api/submit.js` writes through the Git Data API (blob → tree → commit → ref) precisely so they land together; sequential per-file writes would be three commits with no atomicity, and a failure between them leaves the fleet series counting a submission the index does not list. Do not "simplify" this back to the Contents API: besides losing atomicity it reintroduces a hard 1 MB inline-content wall on the read, which fails the write path silently. A moved branch (422 on the ref update) earns exactly one re-read, and the match, the merge and the fleet delta must all re-run against the fresh content — never against the first read.
...
- **Nothing published may be replayable.** Anything a reader of any data file can copy is, by definition, something they can send back. So the index stores a second hash of the client's anchors and only the hash of a link token, and `identity`/`anchor_hashes`/`token_hash`/`updated_at`/`daily_days`/`detail` are server-generated and rejected by the client whitelist. If you add a field that participates in deciding WHICH row a submission may write, it belongs under the same rule and needs a mutant in `tests/mutation_run.py`.
